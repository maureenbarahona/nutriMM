...
export function SiteFooter() {
    ...
    const allLinks = [
        { href: '/scan', label: t('Navigation.scan') },
        { href: '/add', label: t('Navigation.addManually') },
        { href: '/portions', label: t('Navigation.portions'), protected: true },
        { href: '/bmi', label: t('Navigation.bmi'), protected: true },
        { href: '/history', label: t('Navigation.history') },
    ];

    const visibleLinks = allLinks.filter(link => !link.protected || isAuthorized);
        ...
        <footer className="bg-muted/50 text-muted-foreground pb-20 md:pb-10">
            <div className="container py-10 px-5">
                ...
                <div className="border-t border-border mt-5 pt-5 text-center text-sm">
                    <p>&copy; {new Date().getFullYear()} <strong className="font-semibold">Nutri MyM</strong>. Todos los derechos reservados.</p>
